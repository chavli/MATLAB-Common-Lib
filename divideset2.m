...
%       testp - probability an example will be put into the test set [0-1]
%   
%   returns:
...
function [ training_m, testing_m ] = divideset2( data_m, testp_n )
    max_training = ceil( testp_n * length(data_m));
    max_testing = floor( (1 - testp_n) * length(data_m));

    cols = size(data_m, 2);
    training_m = zeros(max_testing, cols);
    testing_m = zeros(max_training, cols);
    ctr_0 = 1; ctr_1= 1;
